...
def parse_notes_bk(
    notes_xml_path: str | Path, front_xml_path: str | Path
) -> list[dict]:
    """Parse TEI textual notes from an XML file into structured dictionaries.

    Each `<note>` element is converted to a dictionary containing its XML attributes
    and the structured content of its child elements. The `wit` field of every reading
    is expanded to the full list of sigla using `parse_witness_formula`.

    Args:
        notes_xml_path (str | Path): Path to the TEI XML textual notes file. The file
            may contain an internal DTD subset with entity declarations — these
            are resolved automatically by the parser.
        front_xml_path (str | Path): Path to the TEI XML file containing the `<listWit>`
            elements that define the witness sigla and their dates.

    Returns:
        list[dict]: One dictionary per `<note>` element.

        * All XML attributes of the `<note>` element.  The `xml:id`
          attribute is normalised from the Clark-notation key
          `{http://www.w3.org/XML/1998/namespace}id` to the string
          `"xml:id"`.
        * `"label"` — text content of the `<label>` child.
        * `"lem"` — text content of the `<lem>` element inside `<app>`.
        * `"readings"` — list of dicts, one per `<appPart>`, each with:

          - `"rdg"` — text of the `<rdg>` element.
          - `"rdg_type"` — value of the `type` attribute on `<rdg>`
            (omitted when absent).
          - `"wit"` — list of sigla returned by `parse_witness_formula`
            (omitted when no `<wit>` element is present).

    Example:
        notes = parse_notes("textual_notes.xml", "front.xml")
        for note in notes:
            print(note["xml:id"], note.get("lem"))
            for reading in note["readings"]:
                print("  ", reading["rdg"], "→", reading.get("wit"))

    # WARNING: The chronological sorting may be affected by the presence of
    # punctuation marks or date ranges in the `date` field of the `<witness>` elements.
    # Some additional parsing may be necessary to ensure accurate sorting if the `date`
    # field contains complex date formats.
    """
    # 1. Gather historical context and build witness collections
    witness_list_tuples = get_witness_list(front_xml_path, show_dates=True)
    witness_dict = dict(
        witness_list_tuples
    )  # Map sigla to dates for sorting and annotation
    witness_list = [
        sig for sig, _ in witness_list_tuples
    ]  # Get a flat list of just the sigla

    # Parse both documents using BeautifulSoup and the lxml XML parser
    with open(notes_xml_path, "r", encoding="utf-8") as f:
        notes_soup = BeautifulSoup(f, "lxml-xml")
    ...
    # 2. Iterate through every <note> tag in the file
    for note_el in notes_soup.find_all("note"):
        d: dict = {}

        # BeautifulSoup stores XML attributes directly. 'xml:id' is retained as a literal key string.
        # This completely replaces the Clark-notation mapping boilerplate.
        for k, v in note_el.attrs.items():
            d[k] = v

        # Extract <label> text safely
        label_el = note_el.find("label")
        if label_el is not None:
            d["label"] = label_el.get_text(strip=True)

        # Process the <app> block containing <lem> and <appPart>
        app_el = note_el.find("app")
        if app_el is not None:
            lem_el = app_el.find("lem")
            if lem_el is not None:
                d["lem"] = lem_el.get_text(strip=True)

            readings: list[dict] = []

            # Find all <appPart> children inside this specific <app> block
            for part in app_el.find_all("appPart"):
                reading: dict = {}

                # Process the inner reading (<rdg>)
                rdg_el = part.find("rdg")
                if rdg_el is not None:
                    reading["rdg"] = re.sub("\\s+", " ", rdg_el.get_text(strip=True))
                    if rdg_el.has_attr("type"):
                ...
                # Process the witness formula (<wit>)
                wit_el = part.find("wit")
                if wit_el is not None:
                    # Update your internal helper signatures to accept BeautifulSoup elements
                    witnesses = parse_witness_formula(wit_el, witness_list)
                    witnesses = _expand_collective_sigla(
                        witnesses, front_soup, witness_list
                    )

                    # Sort dynamically based on the parsed dates mapped earlier
                    witnesses_tuples = smart_date_sorted(
                        [(sig, witness_dict[sig]) for sig in witnesses]
                    )
                    witnesses = [sig for sig, _ in witnesses_tuples]

                    # Apply your custom witness annotations
                    witnesses = _annotate_witnesses(front_soup, witnesses)
                    reading["wit"] = witnesses
